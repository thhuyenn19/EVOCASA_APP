...
export class CategoryComponent implements OnInit {
      ...
      this.categoryService.deleteCategory(categoryId).subscribe({
        next: () => {
          ...
          this.categories = this.categories.filter(
            (c) => this.getCategoryIdAsString(c.id) !== categoryId
          );
  ...
  getParentCategoryName(parentCategoryId: string | { $oid: string }): string {
    ...
    const parentCategory = this.categories.find(
      (category) => this.getCategoryIdAsString(category.id) === parentIdStr
    );
  ...
  exportCategories(): void {
    ...
    const csvRows = categoriesToExport.map((category, index) => [
      ...
      category.parentCategory
        ? `"${this.getParentCategoryName(category.parentCategory)}"`
        : 'Root Category',
